...
export async function prepareEnvelope(env: ChatEnvelope): Promise<PreparedEnvelope> {
  const cpp = await getCpp();
  const jsonBytes = JSON.stringify(env).length;
  const t0 = performance.now();
  const bytes = encodeDynamic(cpp, CHAT_ENVELOPE_SCHEMA, env) as Uint8Array;
  const t1 = performance.now();
  const reader = openDynamic(cpp, CHAT_ENVELOPE_SCHEMA, bytes);
  try { void reader.get("body"); } finally { try { reader.dispose(); } catch { /* best effort */ } }
  const t2 = performance.now();
  return {
    bytes,
    stats: {
...
export async function decodeEnvelope(bytes: Uint8Array): Promise<ChatEnvelope> {
  const cpp = await getCpp();
  const reader = openDynamic(cpp, CHAT_ENVELOPE_SCHEMA, bytes);
  try {
    return {
