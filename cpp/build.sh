...
case "${1:-}" in
  ...
  reader)
    READER_ONLY=1
    echo "[build.sh] CW_READER_ONLY=1 — read path only (no builder/RPC/lazy/tape)"
    ;;
  "")
    ;;
  *)
    echo "[build.sh] unknown mode: $1 (expected: bench | reader | <empty>)" >&2
    exit 2
    ;;
esac
...
# Reader-only mode: drop builder, RPC, lazy reader, and tape codec from the
# wasm. Source is already gated by #ifdef CW_READER_ONLY in cpp/wrapper.cpp;
# we also strip exports that would otherwise force the linker to keep
# unreachable definitions, and we skip rpc.capnp.c++ from the link since
# nothing references it. Result is dist/capnp.reader.wasm — the smallest
# self-contained read path that still supports draft() projections, list
# projection, and per-field getters generated by the codegen.
if [ "$READER_ONLY" = "1" ]; then
  FLAGS+=( -DCW_READER_ONLY=1 )
  # Strip exports for code that's been #ifdef'd out so the linker doesn't
  # emit dangling references. Filter the FLAGS array by rebuilding it
  # without any -Wl,--export entry that names a symbol from a stripped
  # subsystem (lazy reader, builder, RPC).
  reader_strip=$(cat <<'STRIP'
cpp_lazy_open
cpp_lazy_msg_obj_field_text
cpp_lazy_obj_fields_text
cpp_any_builder_init
cpp_any_builder_set_text
cpp_any_builder_set_data
cpp_any_builder_init_list_uint8
cpp_any_builder_set_list_uint8
cpp_any_builder_init_list_uint16
cpp_any_builder_set_list_uint16
cpp_any_builder_init_list_uint32
cpp_any_builder_set_list_uint32
cpp_any_builder_init_list_uint64
cpp_any_builder_set_list_uint64
cpp_any_builder_init_list_int8
cpp_any_builder_set_list_int8
cpp_any_builder_init_list_int16
cpp_any_builder_set_list_int16
cpp_any_builder_init_list_int32
cpp_any_builder_set_list_int32
cpp_any_builder_init_list_int64
cpp_any_builder_set_list_int64
cpp_any_builder_init_list_float32
cpp_any_builder_set_list_float32
cpp_any_builder_init_list_float64
cpp_any_builder_set_list_float64
cpp_any_builder_init_list_bool
cpp_any_builder_set_list_bool
cpp_any_builder_init_list_text
cpp_any_builder_set_list_text
cpp_any_builder_init_list_data
cpp_any_builder_set_list_data
cpp_any_builder_enter_struct
cpp_any_builder_exit_struct
cpp_any_builder_init_list_struct
cpp_any_builder_enter_list_element
cpp_any_builder_finalize
cpp_any_builder_data_ptr
cpp_rpc_build_bootstrap
cpp_rpc_build_call
cpp_rpc_build_return
cpp_rpc_build_release
cpp_rpc_build_disembargo_receiver_loopback
cpp_rpc_get_abort_reason
cpp_rpc_get_resolve_cap_id
cpp_rpc_get_resolve_exception
cpp_rpc_decode
cpp_rpc_get_call_params
cpp_rpc_get_return_results
cpp_rpc_get_return_exception
cpp_rpc_build_return_with_caps
cpp_rpc_get_return_cap_kind
cpp_rpc_get_return_cap_id
cpp_rpc_begin_call
cpp_rpc_begin_return
cpp_rpc_finalize
cpp_rpc_open_call_params
cpp_rpc_open_return_results
STRIP
)
  filtered_flags=()
  for f in "${FLAGS[@]}"; do
    ...
    while IFS= read -r sym; do
      [ -z "$sym" ] && continue
      if [ "$f" = "-Wl,--export=$sym" ]; then keep=0; break; fi
    done <<< "$reader_strip"
    [ "$keep" = "1" ] && filtered_flags+=("$f")
  done
  FLAGS=("${filtered_flags[@]}")
  # rpc.capnp.c++ has no reachable code in reader mode; drop from the link.
  filtered_caps=()
  for s in "${CAPNP_SOURCES[@]}"; do
    [[ "$s" == *"rpc.capnp.c++" ]] && continue
    filtered_caps+=("$s")
  done
  CAPNP_SOURCES=("${filtered_caps[@]}")
fi
...
#   dist/capnp.reader.wasm  — read-path-only wasm (no builder/RPC/lazy/tape).
#                             For consumers that only project responses via
#                             draft(); served by `import "capnwasm/reader"`.
#   dist/capnp.slim.wasm    — production wasm (no test helpers but full
#                             read + write + RPC), served by
#                             `import "capnwasm/browser"`.
#   dist/inlined.mjs        — full wasm (with helpers) embedded in the
#                             single-file Node/Workers bundle, consumed by
#                             the default `import "capnwasm"`.
#
# Building all three requires three compilation passes. A non-bench,
# non-reader invocation produces capnp.slim.wasm, then recursively
# self-invokes for bench (-> inlined.mjs) and reader (-> capnp.reader.wasm).
if [ "$READER_ONLY" = "1" ]; then
  cp "$OPT_OUT" dist/capnp.reader.wasm
  echo "[build.sh] reader-only artifact: dist/capnp.reader.wasm"
elif [ "$BENCH_MODE" = "1" ]; then
  ...
  bash "$0" bench
  echo "[build.sh] building reader-only variant..."
  bash "$0" reader
fi
