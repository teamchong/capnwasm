...
function generateJs(structs, schemaName) {
  ...
  lines.push(`  text: _jsReadTextPtr, data: _jsReadDataPtr,`);
...
function _capnwasmPick(cpp, fields, names) {`);
  ...
  lines.push(`}`);
  lines.push("");
  lines.push(`function _compileDirectListDecoder(descs, names, filter) {`);
  lines.push(`  const cols = descs.length;`);
  lines.push(`  let filterColIdx = -1;`);
  lines.push(`  if (filter) {`);
  lines.push(`    filterColIdx = names.indexOf(filter.field);`);
  lines.push(`    if (filterColIdx < 0 || (descs[filterColIdx] && descs[filterColIdx].type !== "bool")) { filter = null; filterColIdx = -1; }`);
  lines.push(`  }`);
  lines.push(`  const out = [];`);
  lines.push(`  out.push("const text = H.text, data = H.data;");`);
  lines.push(`  out.push("const F32U = H.F32U, F32F = H.F32F, F64U = H.F64U, F64F = H.F64F;");`);
  lines.push(`  out.push("if (start === undefined) start = 0;");`);
  lines.push(`  out.push("if (limit === undefined) limit = list.count;");`);
  lines.push(`  out.push("if (limit > list.count) limit = list.count;");`);
  lines.push(`  out.push("if (start > limit) start = limit;");`);
  lines.push(`  out.push("const arr = new Array(limit - start);");`);
  lines.push(`  if (filter) out.push("let arrIdx = 0;");`);
  lines.push(`  out.push("const step = (list.dataWords + list.ptrWords) * 8;");`);
  lines.push(`  out.push("for (let row = start; row < limit; row++) {");`);
  lines.push(`  out.push("  const dataPtr = list.elementsBase + row * step;");`);
  lines.push(`  if (filter) {`);
  lines.push(`    const fd = descs[filterColIdx];`);
  lines.push(`    const byteOff = fd.off >>> 3;`);
  lines.push(`    const bit = fd.off & 7;`);
  lines.push(`    const predCmp = filter.kind === "truthy" ? "=== 0" : "!== 0";`);
  lines.push(`    out.push("  if (((u8[dataPtr + " + byteOff + "] >> " + bit + ") & 1) " + predCmp + ") continue;");`);
  lines.push(`  }`);
  lines.push(`  for (let col = 0; col < cols; col++) {`);
  lines.push(`    const d = descs[col];`);
  lines.push(`    switch (d.type) {`);
  lines.push(`      case "text":`);
  lines.push(`        out.push("  const _t" + col + " = text(u8, dv, dataPtr, list.dataWords, " + d.off + ", msgStart, msgEnd);");`);
  lines.push(`        out.push("  const _v" + col + " = _t" + col + " === undefined ? undefined : (_t" + col + " ?? \\\"\\\");");`);
  lines.push(`        break;`);
  lines.push(`      case "data":`);
  lines.push(`        out.push("  const _d" + col + " = data(u8, dv, dataPtr, list.dataWords, " + d.off + ", msgStart, msgEnd);");`);
  lines.push(`        out.push("  const _v" + col + " = _d" + col + " === undefined ? undefined : (_d" + col + " ?? new Uint8Array(0));");`);
  lines.push(`        break;`);
  lines.push(`      case "bool": {`);
  lines.push(`        const byteOff = d.off >>> 3;`);
  lines.push(`        const bit = d.off & 7;`);
  lines.push(`        out.push("  const _v" + col + " = ((u8[dataPtr + " + byteOff + "] >> " + bit + ") & 1) === 1;");`);
  lines.push(`        break;`);
  lines.push(`      }`);
  lines.push(`      case "uint8": out.push("  const _v" + col + " = u8[dataPtr + " + d.off + "] >>> 0;"); break;`);
  lines.push(`      case "int8": out.push("  const _v" + col + " = (u8[dataPtr + " + d.off + "] << 24) >> 24;"); break;`);
  lines.push(`      case "uint16": out.push("  const _v" + col + " = dv.getUint16(dataPtr + " + d.off + ", true);"); break;`);
  lines.push(`      case "int16": out.push("  const _v" + col + " = dv.getInt16(dataPtr + " + d.off + ", true);"); break;`);
  lines.push(`      case "uint32": out.push("  const _v" + col + " = dv.getUint32(dataPtr + " + d.off + ", true) >>> 0;"); break;`);
  lines.push(`      case "int32": out.push("  const _v" + col + " = dv.getInt32(dataPtr + " + d.off + ", true);"); break;`);
  lines.push(`      case "float32": out.push("  const _v" + col + " = dv.getFloat32(dataPtr + " + d.off + ", true);"); break;`);
  lines.push(`      case "float64": out.push("  const _v" + col + " = dv.getFloat64(dataPtr + " + d.off + ", true);"); break;`);
  lines.push(`      case "int64":`);
  lines.push(`        out.push("  let _v" + col + ";");`);
  lines.push(`        out.push("  { const _lo = dv.getUint32(dataPtr + " + d.off + ", true); const _hi = dv.getInt32(dataPtr + " + (d.off + 4) + ", true); _v" + col + " = (_hi >= -0x200000 && _hi <= 0x1FFFFF) ? _hi * 4294967296 + _lo : dv.getBigInt64(dataPtr + " + d.off + ", true); }");`);
  lines.push(`        break;`);
  lines.push(`      case "uint64":`);
  lines.push(`        out.push("  let _v" + col + ";");`);
  lines.push(`        out.push("  { const _lo = dv.getUint32(dataPtr + " + d.off + ", true) >>> 0; const _hi = dv.getUint32(dataPtr + " + (d.off + 4) + ", true) >>> 0; _v" + col + " = (_hi <= 0x001FFFFF) ? _hi * 4294967296 + _lo : ((BigInt(_hi) << 32n) | BigInt(_lo)); }");`);
  lines.push(`        break;`);
  lines.push(`      default: out.push("  const _v" + col + " = undefined;");`);
  lines.push(`    }`);
  lines.push(`  }`);
  lines.push(`  const litParts = names.map((n, i) => JSON.stringify(n) + ": _v" + i);`);
  lines.push(`  const targetExpr = filter ? "arr[arrIdx++]" : "arr[row - start]";`);
  lines.push(`  out.push("  " + targetExpr + " = { " + litParts.join(", ") + " };");`);
  lines.push(`  out.push("}");`);
  lines.push(`  if (filter) out.push("arr.length = arrIdx;");`);
  lines.push(`  out.push("return arr;");`);
  lines.push(`  return new Function("u8", "dv", "list", "msgStart", "msgEnd", "H", "start", "limit", out.join("\\n"));`);
  lines.push(`}`);
  lines.push("");
  lines.push(`function _capnwasmListProjectDirect(reader, ptrIndex, meta, names, bounds, filter) {`);
  lines.push(`  if (!reader || !reader._msgEnd || !meta) return null;`);
  lines.push(`  if (filter && names.indexOf(filter.field) < 0) return null;`);
  ...
  lines.push(`  }`);
  lines.push(`  const list = _jsReadListStructPtr(reader._u8, reader._dv, reader._dataPtr, reader.constructor._DATA_WORDS, ptrIndex, reader._msgStart, reader._msgEnd);`);
  lines.push(`  if (list === undefined) return null;`);
  lines.push(`  if (list === null) return [];`);
  lines.push(`  const entry = _getPickRequest(meta.fields, names);`);
  lines.push(`  let start = 0, limit = list.count;`);
  lines.push(`  if (bounds) {`);
  lines.push(`    start = bounds[0] | 0; if (start > list.count) start = list.count;`);
  lines.push(`    const requested = bounds[1];`);
  lines.push(`    if (requested !== Infinity) { const max = requested | 0; if (max < limit - start) limit = start + max; }`);
  lines.push(`  }`);
  lines.push(`  if (!entry.directListDecoders) entry.directListDecoders = new Map();`);
  lines.push(`  const filterKey = filter ? filter.kind + ":" + filter.field : "";`);
  lines.push(`  let dec = entry.directListDecoders.get(filterKey);`);
  lines.push(`  if (!dec) { dec = _compileDirectListDecoder(entry.descs, names, filter); entry.directListDecoders.set(filterKey, dec); }`);
  lines.push(`  return dec(reader._u8, reader._dv, list, reader._msgStart, reader._msgEnd, _LIST_HELPERS, start, limit);`);
  lines.push(`}`);
  lines.push("");
  lines.push(`const _STRUCT_FIELDS = Object.create(null);`);
  lines.push(`const _STRUCT_META = Object.create(null);`);
  ...
  lines.push(`function _runDraft(reader, fields, fn) {`);
  lines.push(`  const cpp = reader._cpp;`);
  ...
  lines.push(`      const innerFields = _STRUCT_FIELDS[item.inner];`);
  lines.push(`      const innerMeta = _STRUCT_META[item.inner];`);
  lines.push(`      const fastFn = item.shapePreserving ? null : item.fn;`);
  lines.push(`      const fast = fastFn ? null : _capnwasmListProjectDirect(reader, desc.off, innerMeta, item.plan.leaf, plan.outerSlice, item.filter);`);
  lines.push(`      if (fast !== null) return fast;`);
  lines.push(`      const fallback = _capnwasmListProject(cpp, desc.off, innerFields, item.plan.leaf, fastFn, plan.outerSlice, item.filter);`);
  ...
  for (const s of structs) {
    ...
    lines.push(`    return _runDraft(this, ${s.name}Reader._FIELDS, fn);`);
    lines.push(`  }`);
    lines.push("");
  ...
  for (const s of structs) {
    ...
    lines.push(`_STRUCT_META[${JSON.stringify(s.name)}] = { fields: ${s.name}Reader._FIELDS, dataWords: ${s.dataWords}, ptrWords: ${s.ptrWords} };`);
  }
