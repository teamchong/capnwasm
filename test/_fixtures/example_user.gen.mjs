...
export class UserReader {
  ...
  get active() {
    return this._dataPtr ? ((this._u8[this._dataPtr + 12] >> 0) & 1) === 1 : this._exp.cpp_any_bool_at(96, 0) === 1;
  }
  get name() {
  ...
  static _FIELDS = {
    ...
    age: {"kind":3,"off":8,"type":"uint32"},
    active: {"kind":5,"off":96,"type":"bool"},
    name: {"kind":0,"off":0,"type":"text"},
    email: {"kind":0,"off":1,"type":"text"},
    bio: {"kind":0,"off":2,"type":"text"},
  };
...
export class GreetingReader {
  ...
  static _FIELDS = {
    message: {"kind":0,"off":0,"type":"text"},
    timestamp: {"kind":4,"off":0,"type":"uint64"},
  };

  pick(names) {
    return GreetingReader._pickImpl(this._cpp, names);
  }

  static _pickImpl(cpp, names) {
    return _capnwasmPick(cpp, GreetingReader._FIELDS, names);
  }
  ...
  get access() {
    if (!this._plan) {
      this._plan = [];
      const recorded = this._plan;
      const fields = GreetingReader._FIELDS;
      this._access = new Proxy(Object.create(null), {
        get(_, name) {
          if (typeof name === "string" && (name in fields)) recorded.push(name);
          return undefined;
  ...
  apply() {
    if (!this._plan || this._plan.length === 0) return {};
    const result = GreetingReader._pickImpl(this._cpp, this._plan);
    this._plan = null;
    this._access = null;
    return result;
  }
  ...
  toObject() {
    return GreetingReader._pickImpl(this._cpp, Object.keys(GreetingReader._FIELDS));
  }
}
...
export class GreetingBuilder {
  ...
  fromObject(o) {
    if (o == null) return this;
    if (o.message !== undefined) this.message = o.message;
    if (o.timestamp !== undefined) this.timestamp = o.timestamp;
    return this;
  }
...
export function openGreeting(cpp, bytes) {
  ...
  cpp._u8.set(bytes, cpp._exports.cpp_in_ptr());
  const dataPtr = cpp._exports.cpp_any_open(bytes.length);
  return new GreetingReader(cpp, dataPtr);
}
