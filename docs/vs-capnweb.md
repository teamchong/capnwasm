...
### JSON.parse vs capnwasm decode (Node 24, in-process)

Numbers from `bench/m8_attribution.mjs` running on Node 24 / Apple Silicon. Same logical content encoded both ways; ns/op is median of 5 trials at 150 ms each after a 200 ms warmup.
...
- **list/dense iteration depends on access style.** Struct-list scans should go through `draft()`, which prepares the row shape and avoids stable per-row Reader objects. Primitive lists should go through `view()`. Pulling row-shaped data through stable reader objects was removed from the generated list API because it fought the memory-mapped model.
